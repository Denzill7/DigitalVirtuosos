...
function Songs({ state, dispatch }: SideNavProps): JSX.Element {
  ...
  let iter = 0
  // If the backend doesn't give us any songs this crashes the code unless we put this check here
  if (songs.size !== 0){
    iter = songs.get(songs.size-1).get('id')
  }
  return (
      ...
      <div className="search">
        <input onChange={(e: React.FormEvent<HTMLInputElement>) => {
          for (let i = 1; i<=iter; i++){
            let mod = document.getElementById(i.toString())
            let songname = mod?.innerText
            let artistname = mod?.lastElementChild?.id
            //console.log(artistname)
            if (mod && songname && artistname){
              if (songname.toLowerCase().includes(e.currentTarget.value) || artistname.toLowerCase().includes(e.currentTarget.value)){
                mod.className = "f6 pointer underline flex items-center no-underline i dim"
              }
              else{
                mod.className = "invisible"
              }
            }
          }
        }}></input>
      ...
      <div id="seven">
        {songs.map(song => (
          ...
          >
            ...
            <span id={song.get('artist')+" "+song.get('album')}>{song.get('songTitle')}</span>
          </div>
        ))}
      </div>
    </Section>
